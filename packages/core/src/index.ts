...
import { __DEFAULT_CSS__, __DEFAULT_TEMPLATE__ } from './template/default'
  ...
  Schema.intersect([
    ...
    Schema.union([
      Schema.object({
        ...
        style: Schema.string().role('textarea').default(__DEFAULT_CSS__).description('直接编辑样式， class 见<a href="https://imagify.koishi.chat/style">文档</a>'),
      }),
      Schema.object({}),
    ])
  ]),
...
export function apply(ctx: Context, config: Config) {
  ...
  async function createPage(template) {
    ...
    await page.setContent(templater(template, {
      style: config.style || __DEFAULT_CSS__,
  ...
  ctx.on('ready', async () => {
    if (config?.cache?.enable) {
      // clean residue cache
      if (config?.cache?.driver === CacheModel.CACHE) await ctx.cache.clear(name)
  ...
  ctx.before('send', async (session, options) => {
    ...
    const verdict = config.rules.every(rule)
    let cached = false
